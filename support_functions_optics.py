...
def process_directory(directory, model, model_identifier, cellpose_diameter=30,  min_samples=15, xi=0.05, min_cluster_size=0.05,image_resize_factor=3):
    """
    ...
    results_dir = os.path.join(directory, f'results_model{model_identifier}_OPTICS')
    ...
    optics_model, number_of_cells = run_cluster_analysis_on_masks(masks, results_dir, min_samples, min_cluster_size)
    ...
    return optics_model, number_of_cells
...
def run_cluster_analysis_on_masks(masks, save_directory, min_samples=15, do_plot=True):
    """
    Adjusted to let OPTICS autonomously determine the clustering structure.
    The min_samples parameter is provided to help control sensitivity to noise,
    while other parameters are left to their default settings to let the algorithm
    adapt to the data's density variations.
    """
    centers = []
    for i, mask in enumerate(masks):
    ...
    optics_model = OPTICS(min_samples=min_samples).fit(data_array)
    ...
    if do_plot:
        ...
        for k in unique_labels:
            ...
            # Note: Ensure 'colors' is correctly sized to match the number of unique_labels
            col = plt.cm.Spectral(k / max(unique_labels))
            
            # Plot points with the assigned color
            ax.scatter(xyz[:, 0], xyz[:, 1], xyz[:, 2], color=col, label=f'Cluster {k}')
                
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        plt.title(f'Clustered Objects: {total_objects} Clusters')
        scatter_plot_path = os.path.join(save_directory, "clustered_objects_OPTICS_3d_plot.png")
        plt.legend(loc='best')
        plt.savefig(scatter_plot_path)
        plt.show()

    # Save total count to a text file
    count_filename = os.path.join(save_directory, 'total_clusters_count_OPTICS.txt')
    with open(count_filename, 'w') as count_file:
        count_file.write(f"Total distinct clusters (excluding noise): {total_objects}\n")
        
    
    return optics_model, total_objects


    print("Error: optics_model does not have reachability_ attribute. Ensure it's an OPTICS model object.")
def plot_reachability(optics_model, save_path):
    
    reachability = optics_model.reachability_[optics_model.ordering_]
    labels = optics_model.labels_[optics_model.ordering_]
    space = np.arange(len(labels))

    plt.figure(figsize=(10, 7))  # Consider setting a figure size
    plt.fill_between(space, reachability, alpha=0.5)
    ...
    # Plot data points based on their cluster label
    for klass in range(0, len(np.unique(labels))):
        if klass == -1:
            # Plot noise points last, if they exist, using 'k.' (black dot)
            color = 'k.'
        else:
            color = colors[klass % len(colors)]
    ...
    if save_path:
        ...
        plt.show()  # Show the plot interactively if no save path is provided
