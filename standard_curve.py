...
cfu_per_ml = np.array([3.4e8, 1.06e9, 2.34e9])  # Replace with your CFU/mL data
od_values = np.array([0.191, 0.575, 0.753])  # Replace with your OD values
...
cfu_range = np.logspace(8, 12, num=100)  # Example range for CFU/mL (1e8 to 1e12)
...
plt.xlabel('CFU/mL')
plt.ylabel('OD')
plt.title('Standard Curve of OD vs CFU/mL (Starting from OD = 0.191)')
plt.legend()
plt.grid(True)
plt.show()
...
# Example projection
input_od = 0.169  # Replace with the OD you want to use for prediction
