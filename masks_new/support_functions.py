...
def save_masks_overlay(images, masks, save_directory, sub_directory):
    # Create image mask overlay and save images

    Path(f"{save_directory}/{sub_directory}/masks").mkdir(parents=True, exist_ok=True)
    Path(f"{save_directory}/{sub_directory}/overlay").mkdir(parents=True, exist_ok=True)

    zip_masks = zip(images, masks)
    labelled_images = [color.label2rgb(mask, img, alpha = 0.2) for img, mask in zip_masks]

    mask_dir = f'{sub_directory}/masks'
    ...
    lab_img_dir = f'{sub_directory}/overlay'
